...
const ProductDetail: React.FC = () => {
  ...
  if (!product) return <div className="flex justify-center items-center h-screen bg-white"><div className="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-red-600"></div></div>;
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <nav className="flex text-sm font-medium text-gray-500">
            ...
            <Link to="/categories" className="hover:text-red-600 transition-colors">{product.category}</Link>
            <ChevronRight size={16} className="mx-2" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="lg:grid lg:grid-cols-2 lg:gap-x-12 lg:items-start">
          ...
          <div className="flex flex-col space-y-4">
            ...
            >
              ...
              {product.sale_price && (
                <span className="absolute top-4 left-4 bg-red-600 text-white text-sm font-bold px-3 py-1 rounded-full shadow-lg z-10">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8">
              ...
              <div className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-baseline gap-3">
                    <span className="text-3xl font-bold text-red-600">{CURRENCY}{sellingPrice.toLocaleString()}</span>
                    {product.sale_price && (
                ...
                {isReseller && (
                  <div className="text-right bg-red-100 px-4 py-2 rounded-lg border border-red-200">
                     <p className="text-xs text-red-800 font-bold uppercase tracking-wide">রিসেলার প্রাইস</p>
                ...
                {product.sizes && product.sizes.length > 0 && (
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button className="text-xs text-red-600 hover:underline">সাইজ গাইড</button>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                <div className="flex items-center border border-gray-300 rounded-full sm:w-32">
                  <button onClick={() => setQty(Math.max(1, qty - 1))} className="p-3 text-gray-600 hover:text-red-600 transition-colors"><Minus size={18} /></button>
                  <input type="text" readOnly value={qty} className="w-full text-center font-bold text-gray-900 bg-transparent outline-none" />
                  <button onClick={() => setQty(qty + 1)} className="p-3 text-gray-600 hover:text-red-600 transition-colors"><Plus size={18} /></button>
                </div>
                <button 
                  onClick={handleAddToCart}
                  disabled={product.stock <= 0}
                  className={`flex-1 py-4 rounded-full font-bold text-lg shadow-lg shadow-red-100 flex items-center justify-center transition-all transform hover:-translate-y-1 ${
                    product.stock > 0 
              ...
              <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 border-t border-gray-100 pt-6">
                <div className="flex items-center"><Shield size={18} className="text-red-500 mr-2"/> ১০০% অরিজিনাল</div>
                <div className="flex items-center"><Truck size={18} className="text-red-500 mr-2"/> দ্রুত ডেলিভারি</div>
                <div className="flex items-center"><CornerUpLeft size={18} className="text-red-500 mr-2"/> ৭ দিনের রিটার্ন</div>
                <div className="flex items-center"><CreditCard size={18} className="text-red-500 mr-2"/> {product.isCodAvailable ? 'COD আছে' : 'COD নেই'}</div>
              </div>
            </div>
          </div>
        </div>
            ...
            {activeTab === 'shipping' && (
              <div className="grid md:grid-cols-2 gap-8 animate-fade-in">
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                  <h3 className="font-bold text-gray-900 mb-4 flex items-center"><Truck className="mr-2 text-red-600" /> শিপিং রেট</h3>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-16 border-t border-gray-200 pt-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">সম্পর্কিত পণ্য</h2>
