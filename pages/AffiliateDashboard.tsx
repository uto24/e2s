...
import { UserRole, Product, WithdrawRequest } from '../types';
import { encryptResellerData } from '../utils/secureLink';

const AffiliateDashboard: React.FC = () => {
  const { user } = useAuth();
  const { submitAffiliateApplication, products, requestWithdraw } = useShop();
  ...
  const handleWithdrawRequest = async (e: React.FormEvent) => {
      ...
      if (isNaN(amount) || amount < 500) {
          alert("মিনিমাম উইথড্র অ্যামাউন্ট ৫০০ টাকা।");
          return;
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 animate-fade-in">
        ...
        {activeTab === 'dashboard' && (
            <>
                ...
                <div className="mb-10 bg-blue-50 rounded-2xl p-6 border border-blue-100 flex flex-col sm:flex-row justify-between items-center">
                    <div>
                        <h3 className="text-lg font-bold text-blue-900 mb-1">টাকা উত্তোলন করুন</h3>
                        <p className="text-sm text-blue-700">আপনার ব্যালেন্স ৫০০ টাকার বেশি হলে উইথড্র করতে পারবেন।</p>
                    </div>
                    <button 
                        onClick={() => setIsWithdrawModalOpen(true)}
                        disabled={user.balance < 500}
                        className="mt-4 sm:mt-0 px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-200"
                    >
                        <Wallet size={18} className="inline mr-2" /> উইথড্র রিকোয়েস্ট
                    </button>
                </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-scale-in">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                                <h3 className="text-lg font-bold text-gray-900">টাকা উত্তোলন</h3>
                                <button onClick={() => setIsWithdrawModalOpen(false)}><X size={20} className="text-gray-400 hover:text-red-500" /></button>
                            </div>
                            <form onSubmit={handleWithdrawRequest} className="p-6 space-y-4">
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1">অ্যামাউন্ট (BDT)</label>
                                    <input 
                                        type="number" 
                                        min="500"
                                        max={user.balance}
                                        ...
                                        placeholder={`Available: ${user.balance}`}
                                        className="w-full p-3 border rounded-xl focus:ring-blue-500"
                                        required
                                    />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1">পেমেন্ট মেথড</label>
                                    <select value={withdrawMethod} onChange={e => setWithdrawMethod(e.target.value)} className="w-full p-3 border rounded-xl focus:ring-blue-500">
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1">অ্যাকাউন্ট নম্বর / ডিটেইলস</label>
                                    <textarea 
                                        value={withdrawDetails}
                                        onChange={e => setWithdrawDetails(e.target.value)}
                                        placeholder="Example: 01700000000 (Personal)"
                                        className="w-full p-3 border rounded-xl focus:ring-blue-500"
                                        required
                                        rows={2}
                                    />
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 mb-10 relative overflow-hidden">
                    ...
                    <div className="relative z-10 max-w-3xl">
                        <div className="flex items-center mb-4">
                            <div className="bg-green-100 p-2 rounded-lg text-green-600 mr-3"><LinkIcon size={20} /></div>
                        ...
                        <div className="bg-gray-50 px-4 py-3 border-r border-gray-200 text-gray-500 select-none hidden sm:block">.../</div>
                        <input type="text" readOnly value={referralLink.split('//')[1]} className="flex-1 block w-full border-0 py-3 pl-4 focus:ring-0 text-gray-900 font-medium"/>
                        <button onClick={() => copyToClipboard(referralLink)} className="bg-gray-900 text-white px-6 py-3 font-bold text-sm hover:bg-gray-800 transition-colors flex items-center"><Copy size={16} className="mr-2" /> কপি</button>
                        </div>
                    </div>
                </div>
            </>
                 ...
                 <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="p-6 border-b border-gray-100 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div className="relative w-full md:w-96">
                            <input type="text" placeholder="পণ্য খুঁজুন..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-300 focus:ring-green-500 focus:border-green-500" />
                            <Search className="absolute left-3 top-2.5 text-gray-400" size={18} />
                        </div>
                        <p className="text-sm text-gray-500">পণ্য নির্বাচন করে "Create Link" এ ক্লিক করুন।</p>
                    </div>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredProducts.map(product => (
                                    <tr key={product.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="px-2 py-1 inline-flex text-xs leading-5 font-bold rounded-full bg-blue-100 text-blue-800">{CURRENCY}{product.wholesalePrice}</span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{CURRENCY}{product.price}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button onClick={() => { setSelectedProduct(product); setCustomPrice(product.price.toString()); setGeneratedLink(''); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="text-white bg-green-600 hover:bg-green-700 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors shadow-sm shadow-green-200">Create Link</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                             ...
                             <div className="p-6 space-y-6">
                                 ...
                                 <div>
                                     ...
                                     <div className="relative">
                                         <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span className="text-gray-500 font-bold">{CURRENCY}</span></div>
                                         <input type="number" value={customPrice} onChange={(e) => { setCustomPrice(e.target.value); setGeneratedLink(''); }} className="w-full pl-8 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-green-500 focus:border-green-500 font-bold text-lg" />
                                     </div>
                                     {parseFloat(customPrice) >= selectedProduct.wholesalePrice ? (
                                        <div className="mt-2 text-sm text-green-600 font-bold flex items-center bg-green-50 p-2 rounded-lg"><CheckCircle size={14} className="mr-2" /> আপনার লাভ: {CURRENCY}{(parseFloat(customPrice) - selectedProduct.wholesalePrice).toFixed(2)}</div>
                                     ) : (
                                        <div className="mt-2 text-sm text-red-500 font-bold flex items-center"><AlertTriangle size={14} className="mr-2" /> মূল্য হোলসেল প্রাইসের চেয়ে কম হতে পারবে না।</div>
                                     )}
                                 </div>
                                 {generatedLink ? (
                                     <div className="animate-fade-in">
                                         ...
                                         <div className="flex rounded-xl border border-green-200 bg-green-50 overflow-hidden">
                                             <input type="text" readOnly value={generatedLink} className="flex-1 p-3 bg-transparent text-sm text-green-800 font-medium focus:outline-none"/>
                                             <button onClick={() => copyToClipboard(generatedLink)} className="bg-green-600 text-white px-4 hover:bg-green-700 transition-colors"><Copy size={18} /></button>
                                         </div>
                                     ...
                                     <button onClick={handleGenerateLink} disabled={!customPrice || parseFloat(customPrice) < selectedProduct.wholesalePrice} className="w-full py-3 bg-gray-900 text-white rounded-xl font-bold hover:bg-gray-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed">লিংক তৈরি করুন</button>
                                 )}
                             </div>
                         </div>
                     </div>
      ...
      return (
          <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 animate-fade-in">
              <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center relative overflow-hidden">
                  ...
                  <div className="w-20 h-20 bg-yellow-50 rounded-full flex items-center justify-center mx-auto mb-6"><Clock size={36} className="text-yellow-500 animate-pulse" /></div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-2">আবেদন জমা হয়েছে</h2>
                  <p className="text-gray-500 mb-8 leading-relaxed">আপনার এফিলিয়েট আবেদনটি বর্তমানে রিভিউতে আছে। আমাদের টিম যাচাই করার পর আপনাকে নোটিফিকেশন পাঠাবে।</p>
                  <div className="bg-slate-50 rounded-xl p-4 text-sm font-medium text-slate-600 border border-slate-100">গড় সময়: ২৪-৪৮ ঘন্টা</div>
              </div>
          </div>
      );
  }
      ...
      return (
          <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 animate-fade-in">
              <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center relative overflow-hidden">
                  ...
                  <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6"><AlertTriangle size={36} className="text-red-500" /></div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-2">আবেদন বাতিল করা হয়েছে</h2>
                  <p className="text-gray-500 mb-8 leading-relaxed">দুঃখিত, আমাদের পলিসির সাথে সামঞ্জস্য না থাকায় আপনার আবেদনটি গ্রহণ করা সম্ভব হয়নি।</p>
                   <button onClick={() => navigate('/contact')} className="w-full py-3 rounded-xl border border-gray-200 font-bold text-gray-700 hover:bg-gray-50 transition-colors">সাপোর্টে যোগাযোগ করুন</button>
              </div>
          </div>
      );
  }
        ...
        <div className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-gray-100">
            <div className="relative bg-slate-900 px-8 py-16 text-center text-white overflow-hidden">
                ...
                <div className="relative z-10 max-w-2xl mx-auto">
                    ...
                    <p className="text-slate-300 text-lg leading-relaxed">আপনার সোশ্যাল মিডিয়া বা ওয়েবসাইট ব্যবহার করে আমাদের পণ্যের প্রচার করুন এবং প্রতিটি সফল বিক্রয়ে আকর্ষণীয় কমিশন জিতে নিন। অথবা রিসেলার হিসেবে নিজের দামে পণ্য বিক্রি করুন।</p>
                </div>
            </div>
                ...
                <div className="grid md:grid-cols-3 gap-8 mb-16">
                     {[
                        ...
                        <div key={i} className="text-center p-6 rounded-2xl bg-slate-50 border border-slate-100">
                            <div className="w-14 h-14 bg-white shadow-sm rounded-full flex items-center justify-center mx-auto mb-4 text-green-600"><step.icon size={24} /></div>
                    ...
                    <form onSubmit={handleApply} className="space-y-6">
                        <div className="grid md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div className="relative">
                                    <select value={formData.paymentMethod} onChange={(e) => setFormData({...formData, paymentMethod: e.target.value})} className="w-full p-4 bg-slate-50 border-transparent rounded-xl focus:bg-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all appearance-none">
                            ...
                            <div>
                                ...
                                <input type="text" required value={formData.accountNumber} onChange={(e) => setFormData({...formData, accountNumber: e.target.value})} placeholder="01XXXXXXXXX" className="w-full p-4 bg-slate-50 border-transparent rounded-xl focus:bg-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-2">প্রমোশন মেথড</label>
                            <textarea required rows={3} value={formData.promotionMethod} onChange={(e) => setFormData({...formData, promotionMethod: e.target.value})} placeholder="আপনি কিভাবে আমাদের পণ্য প্রমোট করবেন?" className="w-full p-4 bg-slate-50 border-transparent rounded-xl focus:bg-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all" />
                        </div>
                        <div>
                            ...
                            <input type="url" value={formData.socialLink} onChange={(e) => setFormData({...formData, socialLink: e.target.value})} placeholder="https://facebook.com/username" className="w-full p-4 bg-slate-50 border-transparent rounded-xl focus:bg-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all" />
                        </div>
                        <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-green-600 text-white rounded-xl font-bold text-lg hover:bg-green-700 transition-all transform hover:-translate-y-1 shadow-lg shadow-green-200 flex justify-center items-center disabled:opacity-70 disabled:cursor-not-allowed">
